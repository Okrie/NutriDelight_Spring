...
public class MainController {
	...
	public String list(HttpServletRequest request, Model model, HttpSession session) throws Exception {
		...
		String monthSales;
		if (searchmonthSales.getMonthSales() == 0) {
			monthSales = "0";
		} else {
			monthSales = formatter
					.format(searchmonthSales.getMonthSales() + (searchsubscribeSalessolo.getSubscribeSales() * 140000));
		}

		// 매출액(연간) 가져오기
		String yearSales = formatter
				.format(searchyearSalessolo.getYearSales() + (searchsubscribeSalessolo.getSubscribeSales() * 140000));
		// 남은 주문 처리 비율 가져오기
